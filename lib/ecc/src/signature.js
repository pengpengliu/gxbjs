...
class Signature {
    ...
    static signBuffer(buf, private_key) {
        ...
        let signature = Signature.signBufferSha256V2(_hash, private_key);
        let times = 0;
    ...
    static signBufferSha256V2(_hash, private_key) {
        var nonce = 0, sig = null, sigDER = null;
        while (true) {
            sig = secp256k1Lib.sign(_hash, private_key.toBuffer(), {
                noncefn: function () {
                    return sha256(new Buffer(_hash + (nonce++)));
                }
            });
