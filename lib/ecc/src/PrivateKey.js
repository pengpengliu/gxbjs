...
class PrivateKey {
    ...
    get_shared_secret_v2 (public_key, legacy = false) {
        public_key = toPublic (public_key);
        let KB = public_key.toUncompressed ().toBuffer ();
        let shared = PublicKey.fromBuffer (secp256k1Lib.ecdhUnsafe (KB, this.toBuffer (), legacy));
        let S = shared.Q.x.toBuffer ({size: 32});
        if (!legacy && S.length < 32) {
            let pad = new Buffer (32 - S.length).fill (0);
            S = Buffer.concat ([pad, S]);
        }
    ...
    get_shared_secret_v2_no_padding (public_key, legacy = false) {
        public_key = toPublic(public_key)
        let KB = public_key.toUncompressed().toBuffer();
        let shared = PublicKey.fromBuffer(secp256k1Lib.ecdhUnsafe( KB,this.toBuffer(), legacy));
        return sha512(shared.Q.x.toBuffer())
    }
