    ...
    finalize() {
        return new Promise((resolve, reject) => {
            ...
            resolve(Apis.instance().db_api().exec("get_objects", [["2.1.0"]]).then((r) => {
                ...
                this.ref_block_num = this.ref_block_num || r[0].head_block_number & 0xFFFF;
                this.ref_block_prefix = this.ref_block_prefix || new Buffer(r[0].head_block_id, "hex").readUInt32LE(4);
    ...
    */
    ...
    set_required_fees(asset_id) {
        ...
        if (asset_id !== "1.3.1") {
            ...
            promises.push(Apis.instance().db_api().exec("get_required_fees", [operations, "1.3.1"]));
        ...
        return Promise.all(promises).then((results) => {
            ...
            let dynamicPromise = (asset_id !== "1.3.1" && asset) ? Apis.instance().db_api().exec("get_objects", [[asset.dynamic_asset_data_id]]) : new Promise(function (resolve, reject) { resolve(); });
    ...
    sign(chain_id = Apis.instance().chain_id) {
        return new Promise(async (resolve, reject)=>{
            ...
            if (!this.signProvider) {
                ...
                try{
                    this.signatures = await this.signProvider(this, chain_id);
                }catch(err){
                    reject(err);
                    return;
                }
