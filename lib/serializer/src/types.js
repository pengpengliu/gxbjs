...
Types.array = function (st_operation, no_sort = false) {
    return {
        fromByteBuffer(b) {
...
export const string_to_name = (str) => {
    ...
    if (i == 12 && len > 12) {
        const char = str[12];
        let symbol = v.to_long(char_to_symbol(char.charCodeAt(0))).toUnsigned();
        name = name.or(symbol.and(0x0F));
    }
    return name;
};

export const name_to_string = (name) => {
 ...
 * @returns {Long}
 */
export const object_id_type = (account_id_str) => {
    let account_id = account_id_str.split(".").map(i => v.to_long(i));
    return account_id[0].shiftLeft(56).or(account_id[1].shiftLeft(48)).or(account_id[2]);
};

export const checksum160 = Types.bytes(20);
